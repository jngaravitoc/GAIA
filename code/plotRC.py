...
"""
# tes ASCII outputt
f=open('vposeq-test2.dat','w')
i=0
while i < ns:
 print >>f, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f" %(poseqos[0,i],poseqos[1,i]
  ,poseqos[2,i],veleqos[0,i],veleqos[1,i],veleqos[2,i]
  ,alpos[i],delos[i],vrados[i],valpos[i]
  ,vdelos[i],rados[i],vxos[i],vyos[i]+220.0,vzos[i],xos[i]-8.0,yos[i],zos[i], xs[i], ys[i])
 i+=1
f.close()
"""
...
plt.hist2d(xos-8.0, yos, bins=90)
plt.xlim([-7.0, 7.0])
plt.show()
